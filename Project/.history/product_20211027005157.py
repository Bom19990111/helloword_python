        ...
        if str(list_product.list_product[i]['Product_name']).upper() in NameProduct or str(list_product.list_product[i]['Brand']).upper() in NameProduct:
            ...
            if(len(listNew) == 0):
                ...
                for i in range(0, len(listNew)):
                    print("ID: ", listNew[i]['Id']),
                    print("Mã sản phẩm: ",
                          listNew[i]['Product_code']),
                    print("Tên sản phẩm: ",
                          listNew[i]['Product_name']),
                    print("Thương hiệu: ", listNew[i]['Brand']),
                    print("Giá: ", listNew[i]['Price']),
                    print("Năm xuất bản: ", listNew[i]['Year']),
                    print("Số lượng: ", listNew[i]['Quantity']),
                    print("Size giày: ", listNew[i]['Size'])
                    print("________________________________")
        


def SortProductNameABC():
        list_product.list_product.sort(key=lambda item: item.get("Product_name"))      
def SortProductNameCBA():
    list_product.list_product.sorted(
