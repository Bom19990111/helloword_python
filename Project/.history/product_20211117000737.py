...
def ExportExecel():
    ...
    #     pd.DataFrame(list_product.list_product).to_excel(
    #         'danhsachsanpham.xlsx', sheet_name='Sheet1', header=True, index=True)
   df = pd.DataFrame(list_product.list_product).T
   df.to_excel(excel_writer="danhsachsanpham.xlsx", header=True, index=True)
