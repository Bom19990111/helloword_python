...
def ExportExecel():
    for i in range(0, len(list_product.list_product)):
        list_pro = [list_product.list_product[i]]
    pd.DataFrame(list_pro).to_excel(
            'danhsachsanpham.xlsx', sheet_name='Sheet1', header=True, index=True)
    print("Xuất file thành công!!".upper())
    print(list_product.list_product[i])


def ImportExecel():
    data = pd.read_excel('danhsachsanpham.xlsx')
    df = pd.DataFrame(
        data, columns=['Id', 'Product_code', 'Product_name', 'Brand', 'Price',
                       'Year', 'Quantity', 'Size', 'Status'])
    print(df)
